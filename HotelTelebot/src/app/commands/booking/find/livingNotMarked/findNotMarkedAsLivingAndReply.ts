...
const findNotMarkedAsLivingAndReply = async (ctx: Context, date: string, originalMessageId?: number) => {
  const arrivals = await BookingService.fetchLivingButNotMarked(date);
...
export { findNotMarkedAsLivingAndReply };
