...
const findNotPrepaidRemindedNotExpiredAndReply = async (
  ctx: Context,
  date = new Date(),
  originalMessageId?: number
) => {
  const notPrePayedBookings = await BookingService.fetchNotPayedBookingsArriveAfter(formatDate(date), true, false);
...
export { findNotPrepaidRemindedNotExpiredAndReply };
