...
1. **UUID as Primary Keys**: Tables like `client`, `room`, `booking` use UUID primary keys for better distribution and flexibility.

2. **Generated Columns**: The `client` table uses a generated `full_name` column that combines first, middle, and last names.

3. **Array Types**: The `client.phones` field uses PostgreSQL's native array type to store multiple phone numbers.

4. **Audit Trail**: The `booking_history` table tracks all changes to bookings, while `audit_log` provides a system-wide audit trail.
